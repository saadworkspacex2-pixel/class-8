...
// Combined subjects for GPA (Bangla = Bangla 1st + Bangla 2nd, English = English 1st + 2nd)
...
export const SUBJECT_CONFIGS: Record<string, SubjectConfig> = {
  ...
  Islam: { name: "Islam", cqMax: 70, mcqMax: 30, totalMax: 100, hasMcq: true },
   ICT: { name: "ICT", cqMax: 35, mcqMax: 15, totalMax: 50, hasMcq: true },
};
...
// Standardized grading scale based on Final Mark (0-100)
export function calculateGradeFromMark(mark: number): { grade: string; gp: number } {
  if (mark >= 80) return { grade: "A+", gp: 5.00 };
  if (mark >= 70) return { grade: "A", gp: 4.00 };
  if (mark >= 60) return { grade: "A-", gp: 3.50 };
  if (mark >= 50) return { grade: "B", gp: 3.00 };
  if (mark >= 40) return { grade: "C", gp: 2.00 };
  if (mark >= 33) return { grade: "D", gp: 1.00 };
  return { grade: "F", gp: 0.00 };
}

// 80/20 weighted calculation: (CQ + MCQ) * 0.80 + Monthly Marks
export function calculateWeightedFinal(cq: number, mcq: number, monthlyMarks: number): {
  examTotal: number;
  weighted80: number;
  finalMark: number;
  grade: string;
  ...
  const finalMark = Math.round((weighted80 + monthlyMarks) * 100) / 100;
  const result = calculateGradeFromMark(finalMark);
  const isPass = result.grade !== "F";
  return { examTotal, weighted80, finalMark, grade: result.grade, gp: result.gp, pass: isPass };
}
